...
__tanzu_init_completion()
{
    COMPREPLY=()
    _get_comp_words_by_ref "$@" cur prev words cword
}

# This function calls the tanzu program to obtain the completion
# results and the directive.  It fills the 'out' and 'directive' vars.
__tanzu_get_completion_results() {
    local requestComp lastParam lastChar args

    # Prepare the command to request completions for the program.
    # Calling ${words[0]} instead of directly tanzu allows handling aliases
    args=("${words[@]:1}")
    requestComp="${words[0]} __complete ${args[*]}"

    lastParam=${words[$((${#words[@]}-1))]}
    lastChar=${lastParam:$((${#lastParam}-1)):1}
    __tanzu_debug "lastParam ${lastParam}, lastChar ${lastChar}"

    if [[ -z ${cur} && ${lastChar} != = ]]; then
        # If the last parameter is complete (there is a space following it)
        # We add an extra empty parameter so we can indicate this to the go method.
        __tanzu_debug "Adding extra empty parameter"
        requestComp="${requestComp} ''"
    fi

    # When completing a flag with an = (e.g., tanzu -n=<TAB>)
    ...
    __tanzu_debug "The completion directive is: ${directive}"
    __tanzu_debug "The completions are: ${out}"
}

__tanzu_process_completion_results() {
    ...
    if (((directive & shellCompDirectiveError) != 0)); then
        ...
        __tanzu_debug "Received error from custom completion go code"
        return
    else
        if (((directive & shellCompDirectiveNoSpace) != 0)); then
            if [[ $(type -t compopt) == builtin ]]; then
                __tanzu_debug "Activating no space"
                compopt -o nospace
            else
                __tanzu_debug "No space directive not supported in this version of bash"
            fi
        fi
        if (((directive & shellCompDirectiveKeepOrder) != 0)); then
            if [[ $(type -t compopt) == builtin ]]; then
                ...
                if [[ ${BASH_VERSINFO[0]} -lt 4 || ( ${BASH_VERSINFO[0]} -eq 4 && ${BASH_VERSINFO[1]} -lt 4 ) ]]; then
                    __tanzu_debug "No sort directive not supported in this version of bash"
                else
                    __tanzu_debug "Activating keep order"
                    compopt -o nosort
                fi
            else
                __tanzu_debug "No sort directive not supported in this version of bash"
        ...
        if (((directive & shellCompDirectiveNoFileComp) != 0)); then
            if [[ $(type -t compopt) == builtin ]]; then
                __tanzu_debug "Activating no file completion"
                compopt +o default
            else
                __tanzu_debug "No file completion directive not supported in this version of bash"
            fi
        fi
    fi
    ...
    if (((directive & shellCompDirectiveFilterFileExt) != 0)); then
        ...
        __tanzu_debug "File filtering command: $filteringCmd"
        $filteringCmd
        ...
        if [[ -n $subdir ]]; then
            __tanzu_debug "Listing directories in $subdir"
            pushd "$subdir" >/dev/null 2>&1 && _filedir -d && popd >/dev/null 2>&1 || return
        else
            __tanzu_debug "Listing directories in ."
            _filedir -d
        fi
    else
        __tanzu_handle_completion_types
    fi

    __tanzu_handle_special_char "$cur" :
    __tanzu_handle_special_char "$cur" =
...
__tanzu_extract_activeHelp() {
    ...
    while IFS='' read -r comp; do
        if [[ ${comp:0:endIndex} == $activeHelpMarker ]]; then
            comp=${comp:endIndex}
            __tanzu_debug "ActiveHelp found: $comp"
            if [[ -n $comp ]]; then
                activeHelp+=("$comp")
            fi
        else
...
__tanzu_handle_completion_types() {
    __tanzu_debug "__tanzu_handle_completion_types: COMP_TYPE is $COMP_TYPE"
    ...
    *)
        ...
        __tanzu_handle_standard_completion_case
        ;;
    esac
}
...
__tanzu_handle_standard_completion_case() {
    ...
    if ((${#COMPREPLY[*]} == 1)); then
        __tanzu_debug "COMPREPLY[0]: ${COMPREPLY[0]}"
        comp="${COMPREPLY[0]%%$tab*}"
        __tanzu_debug "Removed description from single completion, which is now: ${comp}"
        COMPREPLY[0]=$comp
    else # Format the descriptions
        __tanzu_format_comp_descriptions $longest
    fi
}

__tanzu_handle_special_char()
{
    local comp="$1"
    local char=$2
...
__tanzu_format_comp_descriptions()
{
    local tab=$'\t'
    ...
    for ci in ${!COMPREPLY[*]}; do
        ...
        if [[ "$comp" == *$tab* ]]; then
            __tanzu_debug "Original comp: $comp"
            desc=${comp#*$tab}
            comp=${comp%%$tab*}
...
__start_tanzu()
{
    ...
    if declare -F _init_completion >/dev/null 2>&1; then
        ...
        __tanzu_init_completion -n =: || return
    fi

    __tanzu_debug
    __tanzu_debug "========= starting completion logic =========="
    __tanzu_debug "cur is ${cur}, words[*] is ${words[*]}, #words[@] is ${#words[@]}, cword is $cword"

    # The user could have moved the cursor backwards on the command-line.
    # We need to trigger completion from the $cword location, so we need
    # to truncate the command-line ($words) up to the $cword location.
    words=("${words[@]:0:$cword+1}")
    __tanzu_debug "Truncated words[*]: ${words[*]},"
